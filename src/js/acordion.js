...
function acordion() {
    const acordionContainer = document.getElementsByClassName("accordion")

    for (let index = 0; index < acordionContainer.length; index++) {
        const e = acordionContainer[index];
        ...
        function onClickAcordion() {
            if(ulList.className.indexOf("h-0") > -1) {
                ...
                removelass(button, "rotate-180")
                removeClass(ulList, "opacity-100")
                addClass(ulList, "h-0 invisible opacity-0")
            }
        }
        button.addEventListener("click", onClickAcordion)

        e.getElementsByTagName("h5")[0].append(button)
    }
}

if (window.innerWidth < 768) window.addEventListener("load", acordion)